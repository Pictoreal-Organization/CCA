...
class _CreateMeetingScreenState extends State<CreateMeetingScreen>
  ...
  // ---------------------------------------------------------
  // ✅ NEW REUSABLE POPUP DIALOG FUNCTION
  // ---------------------------------------------------------
  void _showStatusDialog({
    ...
    showDialog(
      context: context,
      barrierDismissible: !isError, // Force user to click OK on errors if desired
          ...
          actions: [
            TextButton(
              onPressed: () {
                ...
                if (onConfirm != null) {
                  onConfirm(); // Run success logic (like closing the screen)
                }
              },
              child: const Text(
                "OK",
  ...
  void submit() async {
    ...
    if (!_formKey.currentState!.validate() || dateTime == null) {
      ...
      _showStatusDialog(
        ...
        message: "Please fill in all required fields and ensure a Date/Time is selected.",
        isError: true,
      );
      return;
    }
    ...
    if (meetingScope == 'team-specific' && selectedTeamIds.isEmpty) {
      ...
      _showStatusDialog(
        ...
        message: "You selected 'Team-Specific' but didn't pick any teams. Please select at least one.",
        isError: true,
      );
      return;
    }
    ...
    if (dateTime != null && dateTime!.isBefore(DateTime.now())) {
      ...
      _showStatusDialog(
        title: "Invalid Time",
        message: "You cannot schedule a meeting in the past. Please adjust the time.",
        isError: true,
      );
      return;
    }
    ...
    try {
      ...
      if (isEditMode) {
        ...
        if (mounted) {
          // ✅ Popup for Success (Update)
          _showStatusDialog(
            title: "Success",
            message: "Meeting updated successfully!",
            isError: false,
            onConfirm: () {
              widget.onMeetingCreated(); // Notify dashboard
              Navigator.pop(context); // Close screen
            },
          );
        }
