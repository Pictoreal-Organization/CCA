...
class _TasksListState extends State<TasksList> {
  ...
  // ✅ NEW: Store control status for each task
  Map<String, bool> taskControlMap = {};

  @override
  void initState() {
    super.initState();
    _checkControlForAllTasks();
  }

  @override
  void didUpdateWidget(TasksList oldWidget) {
    super.didUpdateWidget(oldWidget);
    // Re-check control when tasks list changes
    if (oldWidget.tasks != widget.tasks) {
      _checkControlForAllTasks();
    }
  }

  // ✅ NEW: Check control for all tasks
  Future<void> _checkControlForAllTasks() async {
    for (var task in widget.tasks) {
      final taskId = task['_id'];
      try {
        final hasControl = await taskService.checkTaskControl(taskId);
        if (mounted) {
          setState(() {
            taskControlMap[taskId] = hasControl;
          });
        }
      } catch (e) {
        print('Error checking control for task $taskId: $e');
        if (mounted) {
          setState(() {
            taskControlMap[taskId] = false;
          });
        }
      }
    }
  ...
  Widget _buildTaskCard(
    ...
    // ✅ NEW: Check if user has control over this task
    final hasControl = taskControlMap[taskId] ?? false;
        ...
        child: Container(
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (isExpanded) ...[
                      // In the subtask rendering section, replace the existing assignedUser logic
                      // with this updated code:

                      // Around line 620-640, replace the subtask mapping section with:
                      Padding(
                        ...
                        child: Column(
                          ...
                          children: subtasks.map<Widget>((s) {
                            ...
                            // ✅ NEW: Build a string with all assigned member names
                            String assignedText = 'Not Assigned';
                            if (assignedList.isNotEmpty) {
                              final names = assignedList
                                  .map((user) => user['name'] ?? 'Unknown')
                                  .toList();
                              assignedText = 'Assigned to: ${names.join(', ')}';
                            }
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        // ✅ UPDATED: Show all assigned members
                                        Text(
                                          assignedText,
                                          style: TextStyle(
                                            fontSize: 12,
              ...
              // ✅ MODIFIED: Only show Edit/Delete buttons if user has control
              if (hasControl)
                Padding(
                  padding: const EdgeInsets.only(
                    left: 16,
