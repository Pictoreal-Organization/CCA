...
class TaskService {
  ...
  final String baseUrl = "${AppConfig.baseUrl}/api/tasks";
  ...
  Future<List<dynamic>> getTasksByTeam(String teamId) async {
    final token = await _getToken();
    
    final response = await http.get(
      Uri.parse('$baseUrl/team/$teamId'), // backend route
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer $token",
      },
  ...
  Future<List<dynamic>> getGeneralTasks() async {
    // try {
    //   final response = await http.get('/tasks/general');
    //   return response.data;
    // } catch (e) {
    //   throw Exception("Failed to fetch general tasks: $e");
    // }
    final token = await _getToken();
    
    final response = await http.get(
      Uri.parse('$baseUrl/general'), // backend route
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer $token",
      },
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to load general tasks: ${response.body}');
    }
  }
  ...
  Future<void> completeTask(String taskId) async {
    final token = await _getToken();
    final response = await http.put(
      Uri.parse('$baseUrl/complete/$taskId'),
      headers: {
        "Content-Type": "application/json",
        "Authorization": "Bearer $token",
      },
